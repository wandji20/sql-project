...
SELECT constituency,party
  FROM (SELECT constituency,party, votes, RANK() OVER (PARTITION BY constituency ORDER BY votes DESC ) AS posn
          FROM ge
            WHERE constituency BETWEEN 'S14000021' AND 
             'S14000026' AND yr  = 2017
           ORDER BY constituency,votes DESC) AS filtered
WHERE filtered.posn = 1
/* 6  */
...
SELECT name, date, new
   FROM (SELECT name, date, new, RANK() OVER(PARTITION BY name ORDER BY new DESC) posn 
      FROM (SELECT name, DATE_FORMAT( whn,'%Y-%m-%d') date, 
      confirmed - LAG(confirmed, 1) OVER(PARTITION by name 
      ORDER BY confirmed) new
         FROM covid) tab1
   WHERE new>999) tab2
WHERE posn=1 ORDER BY date
/*  */
